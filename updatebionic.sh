...
if [ -d "$R_ENV_PATH" ]; then
  echo "Conda environment $R_ENV_PATH already exists. Skipping creation."
else
  conda create -y -p "$R_ENV_PATH" python r-base=4.3.3 r-irkernel
fi
...
CONDA_PYTHON_PACKAGES="pyspark py4j numpy scipy pandas geopandas plotnine lifelines duckdb polars pyogrio pyproj" # Add others as needed
...
PYTHON_PACKAGES_TO_CHECK=("py4j" "pyspark" "plotnine" "geopandas") # Add other critical packages
