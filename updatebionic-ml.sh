...
conda install -y -c conda-forge --file "$WORK_DIR/requirements-R.txt" || { echo "Error: Failed to install R packages from requirements-R.txt."; exit 1; }
else
echo "Warning: requirements-R.txt not found in $WORK_DIR. Skipping R package installation from file."
fi

# Pre-install RcppTOML via conda to avoid CRAN compilation hang (dependency of reticulate)
echo "Installing RcppTOML via conda-forge (pre-compiled, avoids C++ compilation issues)..."
conda install -y -c conda-forge r-rcpptoml || echo "Warning: r-rcpptoml conda install failed, will fall back to CRAN"
...
# Purpose: Allow user to easily switch back to original Docker conda at /opt/conda (which has pyspark)
